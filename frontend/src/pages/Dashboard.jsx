...
export default function Dashboard() {
  ...
  if (loading) {
    return <div className="flex h-[70vh] items-center justify-center text-text-muted"><Loader2 className="w-12 h-12 animate-spin text-purple-500" /></div>;
  }

  if (error) {
    return <div className="text-red-400 p-6 border border-red-400/20 rounded-2xl bg-red-400/10 flex items-center gap-4"><Activity /> <span>Error loading stats: {error}</span></div>;
  }
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-10">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="glass-panel p-6 flex flex-col justify-between relative overflow-hidden group hover:border-purple-500/50 transition-colors duration-300">
          ...
          <div className="z-10">
            <h3 className="text-text-muted font-medium mb-1">Total Publications</h3>
            <h2 className="text-5xl font-black text-white">{stats?.total?.toLocaleString() || 0}</h2>
          </div>
        </div>

        {/* Top Venue Quick Stat */}
        <div className="glass-panel p-6 flex flex-col justify-between relative overflow-hidden group hover:border-pink-500/50 transition-colors duration-300">
          ...
          <div className="z-10">
            <h3 className="text-text-muted font-medium mb-1">Top Performing Venue</h3>
            <h2 className="text-2xl font-bold text-white leading-tight line-clamp-2">{stats?.topVenues?.[0]?.venue || 'No data'}</h2>
          </div>
        </div>
        ...
        <div className="glass-panel p-6 flex flex-col justify-between relative overflow-hidden group hover:border-blue-500/50 transition-colors duration-300">
          ...
          <div className="z-10">
            <h3 className="text-text-muted font-medium mb-1">Peak Publication Year</h3>
            <h2 className="text-4xl font-bold text-white">{stats?.topYears?.[0]?.year || 'No data'}</h2>
          </div>
        </div>
      </div>
